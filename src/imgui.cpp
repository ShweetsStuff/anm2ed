...
static void _imgui_spritesheet_add(Imgui* self, const std::string& path) {
  ...
  WorkingDirectory workingDirectory(self->anm2->path);
  ...
  Texture texture;
  if (texture_from_path_init(&texture, spritesheetPath)) {
    int id = map_next_id_get(self->anm2->spritesheets);
    ...
    imgui_log_push(self, std::format(IMGUI_LOG_ADD_SPRITESHEET, id, path));
  } else
    imgui_log_push(self, std::format(IMGUI_LOG_ADD_SPRITESHEET_ERROR, path));
}

static bool _imgui_is_window_hovered(void) { return ImGui::IsWindowHovered(ImGuiHoveredFlags_ChildWindows | ImGuiHoveredFlags_AllowWhenBlockedByActiveItem); }
static bool _imgui_is_window_hovered_and_click(void) { return _imgui_is_window_hovered() && ImGui::IsMouseClicked(0); }
static bool _imgui_is_no_click_on_item(void) { return ImGui::IsMouseClicked(0) && !ImGui::IsItemHovered(); }
...
static vec2 _imgui_item_spacing_get(const ImguiItem& self) { return self.itemSpacing.has_value() ? *self.itemSpacing : vec2(ImGui::GetStyle().ItemSpacing); }
static vec2 _imgui_window_padding_get(const ImguiItem& self) {
  return self.windowPadding.has_value() ? *self.windowPadding : vec2(ImGui::GetStyle().WindowPadding);
}
static vec2 _imgui_frame_padding_get(const ImguiItem& self) {
  return self.framePadding.has_value() ? *self.framePadding : vec2(ImGui::GetStyle().WindowPadding);
}

static vec2 _imgui_cursor_position_get(const ImguiItem& self) { return self.cursorPosition.has_value() ? *self.cursorPosition : vec2(ImGui::GetCursorPos()); }

static vec2 _imgui_item_size_get(const ImguiItem& self, ImguiItemType type) {
  vec2 size{};

  if (!self.size.has_value()) {
    switch (type) {
    case IMGUI_ATLAS_BUTTON:
      size = self.size.has_value() ? *self.size : vec2(ATLAS_SIZE(self.atlas));
      break;
    default:
      if (self.rowCount.has_value())
        size.x = (ImGui::GetWindowSize().x - (_imgui_item_spacing_get(self).x * (*self.rowCount + 1))) / *self.rowCount;
      else if (self.isWidthToText)
        size.x = (ImGui::CalcTextSize(self.label_get().c_str()).x + ImGui::GetStyle().FramePadding.x);
      else if (self.isWidthToRegion)
        size.x = ImGui::GetContentRegionAvail().x;
      else
        size.x = ImGui::CalcItemWidth();

      if (self.isHeightToRegion)
        size.y = ImGui::GetContentRegionAvail().y;
      else if (type != IMGUI_BUTTON)
        size.y = ImGui::GetTextLineHeight() + ImGui::GetStyle().FramePadding.y;
      break;
    }
  } else
    size = *self.size;
...
static void _imgui_item_style_push(const ImguiItem& self) {
  if (self.windowPadding.has_value())
    ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, *self.windowPadding);
  if (self.itemSpacing.has_value())
    ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, *self.itemSpacing);
  if (self.font.has_value())
    ImGui::PushFont(*self.font, FONT_SIZE);
}

static void _imgui_item_style_pop(const ImguiItem& self) {
  if (self.windowPadding.has_value())
    ImGui::PopStyleVar();
  if (self.itemSpacing.has_value())
    ImGui::PopStyleVar();
...
static const char* _imgui_float_format_get(const ImguiItem& item, float& value) {
  if (item.isEmptyFormat)
    return "";
  return float_format_get(value);
}
...
static void _imgui_item_pre(const ImguiItem& self, ImguiItemType type) {
  ...
  case IMGUI_COLOR_EDIT:
    ImGui::SetNextItemWidth(_imgui_item_size_get(self, type).x);
    break;
  default:
    break;
  }
  ...
  case IMGUI_BUTTON:
  case IMGUI_ATLAS_BUTTON:
    if (self.border.has_value()) {
      ImGui::PushStyleVar(ImGuiStyleVar_FrameBorderSize, *self.border);
      if (self.color.border.has_value())
        ImGui::PushStyleColor(ImGuiCol_Border, *self.color.border);
    }
    break;
  default:
    break;
  ...
  switch (type) {
  case IMGUI_BEGIN_CHILD:
    if (self.color.normal.has_value())
      ImGui::PushStyleColor(ImGuiCol_ChildBg, *self.color.normal);
    break;
  case IMGUI_SELECTABLE:
    if (self.isSelected && self.color.normal.has_value())
      ImGui::PushStyleColor(ImGuiCol_Header, *self.color.normal);
    break;
  case IMGUI_BUTTON:
  case IMGUI_ATLAS_BUTTON:
    if (self.color.normal.has_value())
      ImGui::PushStyleColor(ImGuiCol_Button, *self.color.normal);
    if (self.color.active.has_value())
      ImGui::PushStyleColor(ImGuiCol_ButtonActive, *self.color.active);
    if (self.color.hovered.has_value())
      ImGui::PushStyleColor(ImGuiCol_ButtonHovered, *self.color.hovered);
    if (self.isSelected) {
      if (self.color.active.has_value())
  ...
  switch (type) {
  case IMGUI_END_WINDOW:
    _imgui_item_style_pop(self);
    break;
  case IMGUI_END_CHILD:
    break;
  default:
    _imgui_item_style_push(self);
    break;
  }
  ...
  case IMGUI_END_CHILD:
    break;
  default:
    ImGui::SetCursorPos(self.cursorPosition.value_or(vec2(ImGui::GetCursorPos())) + self.cursorOffset);
    break;
  }

  switch (type) {
  case IMGUI_BEGIN_WINDOW:
    if (self.position.has_value())
      ImGui::SetNextWindowPos(*self.position);
    if (self.size.has_value())
      ImGui::SetNextWindowSize(*self.size);
    break;
  default:
    break;
  }
...
static void _imgui_item_post(const ImguiItem& self, Imgui* imgui, ImguiItemType type, bool& isActivated) {
  ...
  if (self.is_mnemonic() && !self.isMnemonicDisabled) {
    ...
    float offset = font->CalcTextSizeA(fontSize, FLT_MAX, 0.0f, start, charPointer).x;
    float charWidth = font->CalcTextSizeA(fontSize, FLT_MAX, 0.0f, charPointer, charPointer + 1).x;
  ...
  if (self.isUseItemActivated && !self.isDisabled)
    isActivated = ImGui::IsItemActivated();

  if (imgui->isContextualActionsEnabled && _imgui_chord_pressed(self.chord_get()) &&
      (self.focusWindow.has_value() && _imgui_is_focus_window(*self.focusWindow)))
    if (!self.isDisabled)
      isActivated = true;

  if (!self.tooltip.empty() && ImGui::IsItemHovered(ImGuiHoveredFlags_DelayNormal)) {
    ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, imgui->style.WindowPadding);
    ImGui::SetTooltip("%s", self.tooltip_get().c_str());
    ImGui::PopStyleVar();
  }

  if (isActivated) {
    if (self.snapshotAction.has_value())
...
#define IMGUI_ITEM_BASE(NAME, TYPE, BODY)                                                                                                                      \
...
#define IMGUI_ITEM_VALUE_BASE(NAME, TYPE, VALUE_TYPE, BODY)                                                                                                    \
  static bool NAME(const ImguiItem& self, Imgui* imgui, VALUE_TYPE& inValue) {                                                                                 \
    ImguiItemType type = TYPE;                                                                                                                                 \
    _imgui_item_pre(self, type);                                                                                                                               \
    bool isActivated = false;                                                                                                                                  \
    do {                                                                                                                                                       \
      VALUE_TYPE value = inValue;                                                                                                                              \
      BODY {}                                                                                                                                                  \
      inValue = value;                                                                                                                                         \
    } while (0);                                                                                                                                               \
    _imgui_item_post(self, imgui, type, isActivated);                                                                                                          \
...
#define IMGUI_ITEM_VOID_FUNCTION(NAME, TYPE, BODY) IMGUI_ITEM_BASE(NAME, TYPE, { ([&] { BODY; })(); })
#define IMGUI_ITEM_BOOL_FUNCTION(NAME, TYPE, BODY) IMGUI_ITEM_BASE(NAME, TYPE, { isActivated = ([&] { return BODY; })(); })
#define IMGUI_ITEM_VALUE_FUNCTION(NAME, TYPE, VALUE_TYPE, BODY) IMGUI_ITEM_VALUE_BASE(NAME, TYPE, VALUE_TYPE, { isActivated = ([&] { return BODY; })(); })

#define IMGUI_ITEM_VALUE_CLAMP_FUNCTION(NAME, TYPE, VALUE_TYPE, BODY)                                                                                          \
  IMGUI_ITEM_VALUE_BASE(NAME, TYPE, VALUE_TYPE, {                                                                                                              \
    isActivated = ([&] { return BODY; })();                                                                                                                    \
    if (self.is_range())                                                                                                                                       \
      value = glm::clamp(value, VALUE_TYPE(*self.min), VALUE_TYPE(*self.max));                                                                                 \
  })

#define IMGUI_ITEM_STRING_FUNCTION(NAME, TYPE, BODY)                                                                                                           \
  static bool NAME(const ImguiItem& self, Imgui* imgui, std::string& inValue) {                                                                                \
    ImguiItemType type = TYPE;                                                                                                                                 \
    _imgui_item_pre(self, type);                                                                                                                               \
    bool isActivated = false;                                                                                                                                  \
    do {                                                                                                                                                       \
      ...
      isActivated = ([&] { return BODY; })();                                                                                                                  \
      /* no VALUE_TYPE(self.value) here, strings don’t come from self.value */                                                                                 \
      inValue = value;                                                                                                                                         \
    } while (0);                                                                                                                                               \
    _imgui_item_post(self, imgui, type, isActivated);                                                                                                          \
    return isActivated;                                                                                                                                        \
...
#define IMGUI_ITEM_CUSTOM_FUNCTION(NAME, TYPE, BODY) IMGUI_ITEM_BASE(NAME, TYPE, BODY)
#define IMGUI_ITEM_CUSTOM_VALUE_FUNCTION(NAME, TYPE, VALUE_TYPE, BODY) IMGUI_ITEM_VALUE_BASE(NAME, TYPE, VALUE_TYPE, {BODY})
...
IMGUI_ITEM_VOID_FUNCTION(_imgui_begin_child, IMGUI_BEGIN_CHILD, ImGui::BeginChild(self.label_get().c_str(), *self.size, self.flags, self.windowFlags));
IMGUI_ITEM_VOID_FUNCTION(_imgui_end_child, IMGUI_END_CHILD, ImGui::EndChild());
IMGUI_ITEM_VOID_FUNCTION(_imgui_text, IMGUI_TEXT, ImGui::Text("%s", self.label_get().c_str()));
IMGUI_ITEM_BOOL_FUNCTION(_imgui_button, IMGUI_BUTTON, ImGui::Button(self.label_get().c_str(), _imgui_item_size_get(self, type)));
...
IMGUI_ITEM_VALUE_CLAMP_FUNCTION(_imgui_input_int, IMGUI_INPUT_INT, int,
                                ImGui::InputInt(self.label_get().c_str(), &value, self.step, self.stepFast, self.flags));
...
IMGUI_ITEM_VALUE_CLAMP_FUNCTION(_imgui_input_float, IMGUI_INPUT_FLOAT, float,
                                ImGui::InputFloat(self.label_get().c_str(), &value, self.step, self.stepFast, _imgui_float_format_get(self, value),
                                                  self.flags));
IMGUI_ITEM_VALUE_FUNCTION(_imgui_checkbox, IMGUI_CHECKBOX, bool, ImGui::Checkbox(self.label_get().c_str(), &value));
IMGUI_ITEM_VALUE_FUNCTION(_imgui_slider_float, IMGUI_SLIDER_FLOAT, float,
                          ImGui::SliderFloat(self.label_get().c_str(), &value, *self.min, *self.max, _imgui_float_format_get(self, value), self.flags));
...
IMGUI_ITEM_BOOL_FUNCTION(_imgui_begin, IMGUI_BEGIN_WINDOW, ImGui::Begin(self.label_get().c_str(), nullptr, self.flags));
IMGUI_ITEM_VOID_FUNCTION(_imgui_end, IMGUI_END_WINDOW, ImGui::End());

IMGUI_ITEM_VALUE_FUNCTION(_imgui_radio_button, IMGUI_RADIO_BUTTON, int, ImGui::RadioButton(self.label_get().c_str(), &value, self.value));
...
IMGUI_ITEM_VALUE_FUNCTION(_imgui_drag_float, IMGUI_DRAG_FLOAT, float,
                          ImGui::DragFloat(self.label_get().c_str(), &value, self.speed, *self.min, *self.max, _imgui_float_format_get(self, value)));
IMGUI_ITEM_VALUE_FUNCTION(_imgui_drag_float2, IMGUI_DRAG_FLOAT, vec2,
                          ImGui::DragFloat2(self.label_get().c_str(), value_ptr(value), self.speed, *self.min, *self.max, _imgui_vec2_format_get(self, value)));
...
IMGUI_ITEM_CHECKBOX_VALUE_FUNCTION(_imgui_checkbox_input_int, int, _imgui_input_int(self, imgui, value));
IMGUI_ITEM_CHECKBOX_VALUE_FUNCTION(_imgui_checkbox_drag_float, float, _imgui_drag_float(self, imgui, value));
...
IMGUI_ITEM_CUSTOM_VALUE_FUNCTION(_imgui_combo, IMGUI_COMBO, int, {
  std::vector<const char*> cStrings;
  cStrings.reserve(self.items.size());
  for (auto& [id, string] : self.items)
    cStrings.emplace_back(string.c_str());

  isActivated = ImGui::Combo(self.label_get().c_str(), &value, cStrings.data(), (int)self.items.size());
});

IMGUI_ITEM_CUSTOM_VALUE_FUNCTION(_imgui_selectable_input_text, IMGUI_SELECTABLE, std::string, {
  static std::string buffer{};
  static int id = ID_NONE;
  ImguiItem selectable = self;
...
#define IMGUI_ATLAS_VOID_FUNCTION(NAME, FUNCTION)                                                                                                              \
  static void NAME(const ImguiItem& self, Imgui* imgui) {                                                                                                      \
    vec2 cursorPosition = _imgui_cursor_position_get(self);                                                                                                    \
    _imgui_image(self.copy({.textureID = imgui->resources->atlas.id,                                                                                           \
                            .size = ATLAS_SIZE(self.atlas) * imgui->settings->displayScale,                                                                    \
                            .uvMin = ATLAS_UV_MIN(self.atlas),                                                                                                 \
                            .uvMax = ATLAS_UV_MAX(self.atlas),                                                                                                 \
                            .cursorPosition = cursorPosition}),                                                                                                \
                 imgui);                                                                                                                                       \
                                                                                                                                                               \
    FUNCTION(self.copy({.cursorPosition =                                                                                                                      \
                            cursorPosition + vec2(ATLAS_SIZE(self.atlas).x + (((_imgui_item_spacing_get(self).x + _imgui_frame_padding_get(self).x * 0.5f) *   \
                                                                               imgui->settings->displayScale)),                                                \
                                                  0.0f)}),                                                                                                     \
             imgui);                                                                                                                                           \
  }

#define IMGUI_ATLAS_BOOL_FUNCTION(NAME, FUNCTION)                                                                                                              \
  static bool NAME(const ImguiItem& self, Imgui* imgui) {                                                                                                      \
    vec2 cursorPosition = _imgui_cursor_position_get(self);                                                                                                    \
    _imgui_image(self.copy({.textureID = imgui->resources->atlas.id,                                                                                           \
                            .size = ATLAS_SIZE(self.atlas) * imgui->settings->displayScale,                                                                    \
                 ...
                 imgui);                                                                                                                                       \
                                                                                                                                                               \
    return FUNCTION(self.copy({.cursorPosition = cursorPosition + vec2(ATLAS_SIZE(self.atlas).x +                                                              \
                                                                           (((_imgui_item_spacing_get(self).x + _imgui_frame_padding_get(self).x * 0.5f) *     \
                                                                             imgui->settings->displayScale)),                                                  \
                                                                       0.0f)}),                                                                                \
                    imgui);                                                                                                                                    \
  }

#define IMGUI_ATLAS_VALUE_BOOL_FUNCTION(NAME, FUNCTION, TYPE)                                                                                                  \
  static bool NAME(const ImguiItem& self, Imgui* imgui, TYPE& value) {                                                                                         \
    vec2 cursorPosition = _imgui_cursor_position_get(self);                                                                                                    \
    _imgui_image(self.copy({.textureID = imgui->resources->atlas.id,                                                                                           \
                            .size = ATLAS_SIZE(self.atlas) * imgui->settings->displayScale,                                                                    \
                            .uvMin = ATLAS_UV_MIN(self.atlas),                                                                                                 \
...
IMGUI_ATLAS_VOID_FUNCTION(_imgui_atlas_text, _imgui_text);
IMGUI_ATLAS_BOOL_FUNCTION(_imgui_atlas_selectable, _imgui_selectable);
IMGUI_ATLAS_VALUE_BOOL_FUNCTION(_imgui_atlas_selectable_input_text, _imgui_selectable_input_text, std::string);

#define IMGUI_BEGIN_OR_RETURN(item, imgui)                                                                                                                     \
  if (!_imgui_begin(item, imgui)) {                                                                                                                            \
    _imgui_end(item, imgui);                                                                                                                                   \
    return;                                                                                                                                                    \
  }
...
static void _imgui_table_set_column_index(int index) { ImGui::TableSetColumnIndex(index); }
...
IMGUI_ITEM_CUSTOM_FUNCTION(_imgui_atlas_button, IMGUI_ATLAS_BUTTON, {
  ...
  if (self.size.has_value()) {
    ...
    vec2 framePadding = _imgui_frame_padding_get(self) * self.scale;

    vec2 atlasOffset = self.atlasOffset.has_value() ? *self.atlasOffset * self.scale : framePadding;
    vec2 start = ImGui::GetItemRectMin() + atlasOffset;
    vec2 atlasSize = self.isAtlasStretch ? *self.size - framePadding : ATLAS_SIZE(self.atlas);
    vec2 end = start + atlasSize;
...
/*
static bool _imgui_selectable_input_int(const ImguiItem& self, Imgui* imgui, int& value) {
  static int temp;
  static int id = ID_NONE;
...
static bool _imgui_confirm_popup(ImguiItem self, Imgui* imgui, ImguiPopupState* state = nullptr, bool isOnlyConfirm = false) {
  ...
  if (state)
    *state = IMGUI_POPUP_STATE_CLOSED;

  if (imgui_begin_popup_modal(self.label_get().c_str(), imgui)) {
    if (state)
      *state = IMGUI_POPUP_STATE_OPEN;

    ImGui::Text("%s", self.text_get());
    ImGui::Separator();
    ...
    if (_imgui_button(IMGUI_POPUP_OK.copy({.rowCount = isOnlyConfirm ? 1 : IMGUI_CONFIRM_POPUP_ROW_COUNT}), imgui)) {
      ...
      if (state)
        *state = isOnlyConfirm ? IMGUI_POPUP_STATE_CANCEL : IMGUI_POPUP_STATE_CONFIRM;
      return true;
    }
    ...
    if (!isOnlyConfirm) {
      if (_imgui_button(IMGUI_POPUP_CANCEL, imgui)) {
        imgui_close_current_popup(imgui);
        if (state)
          *state = IMGUI_POPUP_STATE_CANCEL;
      }
    }
...
static void _imgui_no_anm2_path_click_check(Imgui* self) {
  if (_imgui_is_window_hovered_and_click() && self->anm2->path.empty() && !imgui_is_any_popup_open())
    imgui_open_popup(IMGUI_NO_ANM2_PATH_CONFIRMATION.label);
...
static void _imgui_context_menu(Imgui* self) {
  ...
  if (imgui_begin_popup(IMGUI_CONTEXT_MENU.label_get(), self)) {
    if (_imgui_selectable(IMGUI_CUT, self)) {
      if (self->clipboard->type == CLIPBOARD_ANIMATION)
        self->selectedAnimationIndices = {};
    }
    _imgui_selectable(IMGUI_COPY, self);
    _imgui_selectable(IMGUI_PASTE.copy({.isDisabled = !clipboard_is_value()}), self);
...
static void _imgui_timeline(Imgui* self) {
  ...
  static float& time = self->preview->time;
  ...
  int length = animation->frameNum;
  int actualLength = anm2_animation_length_get(animation);
  ...
  std::function<void()> timeline_header = [&]() {
    ...
    frameTime = (int)(localMousePos.x / frameSize.x);
    ...
    int start = (int)std::floor(scroll.x / frameSize.x) - 1;
    start = (start < 0) ? 0 : start;

    int end = (int)std::ceil((scroll.x + viewWidth) / frameSize.x) + 1;
    ...
    for (int i = start; i < end; i++) {
      ...
      float startX = cursorPos.x + i * frameSize.x;
      float endX = startX + frameSize.x;
      ImVec2 positionStart(startX, cursorPos.y);
      ImVec2 positionEnd(endX, cursorPos.y + frameSize.y);

      ImU32 bgColor =
          isInactive ? (isMultiple ? headerFrameMultipleInactiveColor : headerFrameInactiveColor) : (isMultiple ? headerFrameMultipleColor : headerFrameColor);
  ...
  std::function<void(Anm2Reference, int&)> timeline_item_child = [&](Anm2Reference reference, int& index) {
    Anm2Item* item = anm2_item_from_reference(self->anm2, reference);
    Anm2Type& type = reference.itemType;
    if (!item)
      return;
    if (!self->settings->timelineIsShowUnused && item->frames.empty() && (type == ANM2_LAYER || type == ANM2_NULL))
      return;
    ...
    int buttonCount = type == ANM2_NULL ? 2 : 1;
    float buttonAreaWidth = buttonCount * buttonSize.x + (buttonCount - 1) * defaultItemSpacing.x;
    ...
    const ImguiItem* selectable = IMGUI_TIMELINE_ITEM_SELECTABLES[type];

    std::string dragDrop = selectable->drag_drop_get();
    ...
    _imgui_selectable(selectable->copy({.isSelected = isSelected}), self);
    ...
    if (imgui_begin_popup_modal(IMGUI_POPUP_ITEM_PROPERTIES, self, IMGUI_POPUP_ITEM_PROPERTIES_SIZE)) {
      ...
      _imgui_radio_button(IMGUI_TIMELINE_ITEM_PROPERTIES_LAYER.copy({.isDisabled = true}), self, (int&)type);
      _imgui_radio_button(IMGUI_TIMELINE_ITEM_PROPERTIES_NULL.copy({.isDisabled = true}), self, (int&)type);
      ...
      default: {
        for (auto& [id, layer] : self->anm2->layers) {
          if (id == reference.itemID)
            continue;

          ImGui::PushID(id);

          ImguiItem layerItem = IMGUI_LAYER.copy({.label = std::format(IMGUI_LAYER_FORMAT, id, layer.name), .isSelected = selectedLayerID = id, .id = id});
          if (_imgui_atlas_selectable(layerItem, self))
            selectedLayerID = id;
      ...
      case ANM2_NULL: {
        for (auto& [id, null] : self->anm2->nulls) {
          ...
          ImguiItem nullItem = IMGUI_NULL.copy({.label = std::format(IMGUI_NULL_FORMAT, id, null.name), .isSelected = selectedNullID == id, .id = id});
          if (_imgui_atlas_selectable(nullItem, self))
            selectedNullID = id;
      ...
      if (self->anm2->layers.size() == 0)
        selectedLayerID = ID_NONE;
      if (self->anm2->nulls.size() == 0)
        selectedNullID = ID_NONE;

      bool isDisabled = type == ANM2_NONE || (type == ANM2_LAYER && selectedLayerID == ID_NONE) || (type == ANM2_NULL && selectedNullID == ID_NONE);

      if (_imgui_button(IMGUI_TIMELINE_ITEM_PROPERTIES_CONFIRM.copy({.isDisabled = isDisabled}), self)) {
        switch (type) {
        case ANM2_LAYER:
    ...
    if (_imgui_atlas_button(visibleItem, self))
  ...
  std::function<void()> timeline_items_child = [&]() {
    static int& animationID = self->reference->animationID;
    int index = 0;
    ...
    timeline_item_child({animationID, ANM2_TRIGGER}, index);
    ...
    if (isItemSwap) {
      ...
      swapItemReference = Anm2Reference();
      isItemSwap = false;
    }
  };

  std::function<void(Anm2Reference, int&)> timeline_item_frames = [&](Anm2Reference reference, int& index) {
    Anm2Item* item = anm2_item_from_reference(self->anm2, reference);
    if (!item)
      return;
    Anm2Type& type = reference.itemType;
    ...
    if (_imgui_is_window_hovered()) {
      ...
      self->clipboard->destination = hoverReference;
      self->clipboard->type = CLIPBOARD_FRAME;

      if (ImGui::IsMouseReleased(ImGuiMouseButton_Left)) {
        *self->reference = reference;
        if (reference.itemType == ANM2_LAYER && reference.itemID != ID_NONE)
          self->editor->spritesheetID = self->anm2->layers[self->reference->itemID].spritesheetID;
      }
    }
    ...
    std::function<void(int, Anm2Frame&)> timeline_item_frame = [&](int i, Anm2Frame& frame) {
      static int frameDelayStart{};
      static float frameDelayTimeStart{};
      const bool isModCtrl = ImGui::IsKeyDown(IMGUI_INPUT_CTRL);
      const bool isModAlt = ImGui::IsKeyDown(IMGUI_INPUT_ALT);
      ...
      ImguiItem frameButton = IMGUI_TIMELINE_FRAMES[type]->copy(
          {.atlas = atlas, .size = vec2(frameSize.x * frame.delay, frameSize.y), .isSelected = reference == *self->reference, .id = i});
      ...
      if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_SourceNoPreviewTooltip)) {
        ...
        if (!isModCtrl && type != ANM2_TRIGGER) {
          ImGui::BeginTooltip();
          _imgui_atlas_button(frameButton, self);
          ImGui::EndTooltip();
        }
        ImGui::EndDragDropSource();
      }
      ...
      if (isDrag) {
        self->pendingCursor = SDL_SYSTEM_CURSOR_POINTER;
        ...
        if (Anm2Frame* referenceFrame = anm2_frame_from_reference(self->anm2, *self->reference)) {
          ...
          case ANM2_NULL: {
            if (isModCtrl)
              referenceFrame->delay = std::max(frameDelayStart + (int)(frameTime - frameDelayTimeStart), ANM2_FRAME_NUM_MIN);
            break;
          }
          default:
            break;
      ...
      if (isDrag && ImGui::IsMouseReleased(0)) {
        if (!isModCtrl && *self->reference != hoverReference && self->reference->itemType == hoverReference.itemType) {
          imgui_snapshot(self, IMGUI_ACTION_FRAME_MOVE);
          if (Anm2Frame* referenceFrame = anm2_frame_from_reference(self->anm2, *self->reference)) {
            Anm2Reference addReference = hoverReference;
            addReference.frameIndex =
                hoverReference.frameIndex < self->reference->frameIndex ? std::min(0, addReference.frameIndex - 2) : std::min(0, addReference.frameIndex - 1);
  ...
  std::function<void()> timeline_frames_child = [&]() {
    int& animationID = self->reference->animationID;
    int index = 0;
    ...
    timeline_item_frames(Anm2Reference(animationID, ANM2_TRIGGER), index);
  ...
  if (_imgui_atlas_button(unusedItem, self))
    self->settings->timelineIsShowUnused = !self->settings->timelineIsShowUnused;
  ImGui::PopStyleVar(2);
  ...
  if (imgui_begin_popup_modal(IMGUI_TIMELINE_ADD_ITEM.popup, self, IMGUI_TIMELINE_ADD_ITEM.popupSize)) {
    static int selectedLayerID = ID_NONE;
    static int selectedNullID = ID_NONE;
    int& type = self->settings->timelineAddItemType;
  ...
  if (_imgui_button(IMGUI_TIMELINE_REMOVE_ITEM.copy({.isDisabled = !item || itemType == ANM2_ROOT || itemType == ANM2_TRIGGER}), self)) {
    switch (itemType) {
    case ANM2_LAYER:
      anm2_animation_layer_animation_remove(animation, itemID);
      break;
    case ANM2_NULL:
      anm2_animation_null_animation_remove(animation, itemID);
      break;
    default:
      break;
    }

    *self->reference = {self->reference->animationID};
  }
  ...
  if (_imgui_button(IMGUI_INSERT_FRAME.copy({.isDisabled = !item}), self)) {
    Anm2Reference frameReference = *self->reference;
    Anm2Frame addFrame = Anm2Frame();

    switch (self->reference->itemType) {
    case ANM2_ROOT:
    case ANM2_LAYER:
    case ANM2_NULL:
      frameReference.frameIndex = item->frames.empty() ? 0 : std::clamp(frameReference.frameIndex, 0, (int)(item->frames.size() - 1));

      addFrame = *anm2_frame_from_reference(self->anm2, frameReference);
      break;
    case ANM2_TRIGGER:
      addFrame.atFrame = (int)self->preview->time;
      break;
    default:
      break;
    }
  ...
  if (_imgui_button(IMGUI_REMOVE_FRAME.copy({.isDisabled = !frame}), self)) {
    anm2_frame_remove(self->anm2, *self->reference);
    self->reference->frameIndex = ID_NONE;
  }

  _imgui_button(IMGUI_BAKE.copy({.isDisabled = !frame || itemType == ANM2_TRIGGER}), self);

  if (imgui_begin_popup_modal(IMGUI_BAKE.popup, self, IMGUI_BAKE.popupSize)) {
    static int& interval = self->settings->bakeInterval;
...
static void _imgui_onionskin(Imgui* self) {
  ...
  bool isEnabled = self->settings->onionskinIsEnabled;
  int drawOrder = self->settings->onionskinDrawOrder;
  ...
  auto onionskin_section = [&](auto& text, auto& count, auto& colorOffset) {
    ...
    _imgui_input_int(IMGUI_ONIONSKIN_COUNT.copy({.isDisabled = !isEnabled}), self, count);
    _imgui_color_edit3(IMGUI_ONIONSKIN_COLOR_OFFSET.copy({.isDisabled = !isEnabled}), self, colorOffset);
    ImGui::PopID();
  };

  onionskin_section(IMGUI_ONIONSKIN_BEFORE, self->settings->onionskinBeforeCount, self->settings->onionskinBeforeColorOffset);
  onionskin_section(IMGUI_ONIONSKIN_AFTER, self->settings->onionskinAfterCount, self->settings->onionskinAfterColorOffset);

  ImGui::Separator();

  _imgui_text(IMGUI_ONIONSKIN_DRAW_ORDER, self);
  _imgui_radio_button(IMGUI_ONIONSKIN_BELOW.copy({.isDisabled = !isEnabled}), self, drawOrder);
  _imgui_radio_button(IMGUI_ONIONSKIN_ABOVE.copy({.isDisabled = !isEnabled}), self, drawOrder);

  _imgui_end(IMGUI_ONIONSKIN, self);
}

static void _imgui_taskbar(Imgui* self) {
  static ImguiPopupState exitConfirmState = IMGUI_POPUP_STATE_CLOSED;
  static float displayScale = self->settings->displayScale;
  ImGuiViewport* viewport = ImGui::GetMainViewport();

  ImguiItem taskbar = IMGUI_TASKBAR.copy({.position = viewport->Pos, .size = vec2(viewport->Size.x, IMGUI_TASKBAR.size->y * displayScale)});
  _imgui_begin(taskbar, self);

  Anm2Animation* animation = anm2_animation_from_reference(self->anm2, *self->reference);
  Anm2Item* item = anm2_item_from_reference(self->anm2, *self->reference);

  _imgui_selectable(IMGUI_FILE.copy({.isSelected = imgui_is_popup_open(IMGUI_FILE.popup)}), self);

  if (imgui_begin_popup(IMGUI_FILE.popup, self)) {
    _imgui_selectable(IMGUI_NEW, self);
    _imgui_selectable(IMGUI_OPEN, self);
    _imgui_selectable(IMGUI_SAVE.copy({.isDisabled = self->anm2->path.empty()}), self);
    _imgui_selectable(IMGUI_SAVE_AS.copy({.isDisabled = self->anm2->path.empty()}), self);
    _imgui_selectable(IMGUI_EXPLORE_ANM2_LOCATION.copy({.isDisabled = self->anm2->path.empty()}), self);
  ...
  if (self->isTryQuit && !imgui_is_popup_open(IMGUI_EXIT_CONFIRMATION.label))
    imgui_open_popup(IMGUI_EXIT_CONFIRMATION.label);

  _imgui_confirm_popup(IMGUI_EXIT_CONFIRMATION, self, &exitConfirmState);

  switch (exitConfirmState) {
  case IMGUI_POPUP_STATE_CONFIRM:
    self->isQuit = true;
    break;
  case IMGUI_POPUP_STATE_CANCEL:
  ...
  _imgui_selectable(IMGUI_WIZARD.copy({.isSelected = imgui_is_popup_open(IMGUI_WIZARD.popup)}), self);

  if (imgui_begin_popup(IMGUI_WIZARD.popup, self)) {
    _imgui_selectable(IMGUI_GENERATE_ANIMATION_FROM_GRID.copy({.isDisabled = !item || (self->reference->itemType != ANM2_LAYER)}), self);
    _imgui_selectable(IMGUI_CHANGE_ALL_FRAME_PROPERTIES.copy({.isDisabled = !item}), self);
    _imgui_selectable(IMGUI_SCALE_ANM2.copy({.isDisabled = self->anm2->animations.empty()}), self);
    _imgui_selectable(IMGUI_RENDER_ANIMATION.copy({.isDisabled = !animation}), self);
  ...
  if (imgui_begin_popup_modal(IMGUI_GENERATE_ANIMATION_FROM_GRID.popup, self, IMGUI_GENERATE_ANIMATION_FROM_GRID.popupSize)) {
    ...
    static float& time = self->generatePreview->time;
    ...
    _imgui_end_child(IMGUI_GENERATE_ANIMATION_FROM_GRID_OPTIONS_CHILD, self);
    ...
    _imgui_end_child(IMGUI_GENERATE_ANIMATION_FROM_GRID_SLIDER_CHILD, self);
    _imgui_end_child(IMGUI_GENERATE_ANIMATION_FROM_GRID_PREVIEW_CHILD, self);
    ...
    if (_imgui_button(IMGUI_POPUP_CANCEL, self))
      imgui_close_current_popup(self);
    _imgui_end_child(IMGUI_FOOTER_CHILD, self);
  ...
  if (imgui_begin_popup_modal(IMGUI_CHANGE_ALL_FRAME_PROPERTIES.popup, self, IMGUI_CHANGE_ALL_FRAME_PROPERTIES.popupSize)) {
    ...
    int start = std::max(self->reference->frameIndex, 0);
    int max = isFromSelectedFrame ? std::max(ANM2_FRAME_NUM_MIN, (int)item->frames.size() - start) : (int)item->frames.size();

    auto change_frames = [&](Anm2ChangeType type) {
      Anm2FrameChange frameChange = {
          .isVisible = isVisibleSet ? std::optional{isVisible} : std::nullopt,
          .isInterpolated = isInterpolatedSet ? std::optional{isInterpolated} : std::nullopt,
          .rotation = isRotation ? std::optional{rotation} : std::nullopt,
          .delay = isDelay ? std::optional{delay} : std::nullopt,
          .crop = isCrop ? std::optional{crop} : std::nullopt,
          .pivot = isPivot ? std::optional{pivot} : std::nullopt,
          .position = isPosition ? std::optional{position} : std::nullopt,
          .size = isSize ? std::optional{size} : std::nullopt,
          .scale = isScale ? std::optional{scale} : std::nullopt,
          .offsetRGB = isColorOffset ? std::optional{colorOffset} : std::nullopt,
          .tintRGBA = isTint ? std::optional{tint} : std::nullopt,
      };
      anm2_item_frame_set(self->anm2, *self->reference, frameChange, type, start, numberFrames);
    ...
    /*
    _imgui_checkbox_drag_float2(IMGUI_FRAME_PROPERTIES_CROP.copy({.isDisabled = !isCrop}), self, crop, isCrop);
    _imgui_checkbox_drag_float2(IMGUI_FRAME_PROPERTIES_SIZE.copy({.isDisabled = !isSize}), self, size, isSize);
    _imgui_checkbox_drag_float2(IMGUI_FRAME_PROPERTIES_POSITION.copy({.isDisabled = !isPosition}), self, position, isPosition);
    _imgui_checkbox_drag_float2(IMGUI_FRAME_PROPERTIES_PIVOT.copy({.isDisabled = !isPivot}), self, pivot, isPivot);
    _imgui_checkbox_drag_float2(IMGUI_FRAME_PROPERTIES_SCALE.copy({.isDisabled = !isScale}), self, scale, isScale);
    _imgui_checkbox_drag_float(IMGUI_FRAME_PROPERTIES_ROTATION.copy({.isDisabled = !isRotation}), self, rotation, isRotation);
    _imgui_checkbox_input_int(IMGUI_FRAME_PROPERTIES_DELAY.copy({.isDisabled = !isDelay}), self, delay, isDelay);
    _imgui_checkbox_color_edit4(IMGUI_FRAME_PROPERTIES_TINT.copy({.isDisabled = !isTint}), self, tint, isTint);
    _imgui_checkbox_color_edit3(IMGUI_FRAME_PROPERTIES_COLOR_OFFSET.copy({.isDisabled = !isColorOffset}), self, colorOffset, isColorOffset);
    _imgui_checkbox_checkbox(IMGUI_FRAME_PROPERTIES_VISIBLE.copy({.isDisabled = !isVisibleSet}), self, isVisible, isVisibleSet);
    ImGui::NewLine();
    _imgui_checkbox_checkbox(IMGUI_FRAME_PROPERTIES_INTERPOLATED.copy({.isDisabled = !isInterpolatedSet}), self, isInterpolated, isInterpolatedSet);
   */
    _imgui_end_child(IMGUI_FOOTER_CHILD, self);
    ...
    _imgui_end_child(IMGUI_CHANGE_ALL_FRAME_PROPERTIES_SETTINGS_CHILD, self);

    _imgui_begin_child(IMGUI_FOOTER_CHILD, self);
    if (_imgui_button(IMGUI_CHANGE_ALL_FRAME_PROPERTIES_ADD, self))
      change_frames(ANM2_CHANGE_ADD);
    if (_imgui_button(IMGUI_CHANGE_ALL_FRAME_PROPERTIES_SUBTRACT, self))
      change_frames(ANM2_CHANGE_SUBTRACT);
    if (_imgui_button(IMGUI_CHANGE_ALL_FRAME_PROPERTIES_SET, self))
      change_frames(ANM2_CHANGE_SET);
    if (_imgui_button(IMGUI_CHANGE_ALL_FRAME_PROPERTIES_CANCEL, self))
      imgui_close_current_popup(self);
    _imgui_end_child(IMGUI_FOOTER_CHILD, self);
  ...
  if (imgui_begin_popup_modal(IMGUI_SCALE_ANM2.popup, self, IMGUI_SCALE_ANM2.popupSize)) {
    ...
    _imgui_end_child(IMGUI_SCALE_ANM2_OPTIONS_CHILD, self);
  ...
  if (imgui_begin_popup_modal(IMGUI_RENDER_ANIMATION.popup, self, IMGUI_RENDER_ANIMATION.popupSize)) {
    ...
    static int& type = self->settings->renderType;
    static float& scale = self->settings->renderScale;
    ...
    */
    ...
    _imgui_input_text(IMGUI_RENDER_ANIMATION_FORMAT.copy({.isDisabled = type != RENDER_PNG}), self, format);
    _imgui_checkbox(IMGUI_RENDER_ANIMATION_IS_USE_ANIMATION_BOUNDS, self, isUseAnimationBounds);
    _imgui_input_float(IMGUI_RENDER_ANIMATION_SCALE.copy({.isDisabled = !isUseAnimationBounds}), self, scale);

    _imgui_end_child(IMGUI_RENDER_ANIMATION_CHILD, self);
    ...
    _imgui_end_child(IMGUI_RENDER_ANIMATION_FOOTER_CHILD, self);
  ...
  if (imgui_begin_popup_modal(IMGUI_RENDER_ANIMATION_CONFIRM.popup, self, IMGUI_RENDER_ANIMATION_CONFIRM.popupSize)) {
    ...
    float progress = self->preview->time / (animation->frameNum - 1);
    ImGui::ProgressBar(progress);

    _imgui_text(IMGUI_RENDERING_ANIMATION_INFO, self);

    _imgui_end_child(IMGUI_RENDERING_ANIMATION_CHILD, self);
  ...
  _imgui_selectable(IMGUI_PLAYBACK.copy({.isSelected = imgui_is_popup_open(IMGUI_PLAYBACK.popup)}), self);
  ...
  _imgui_selectable(IMGUI_SETTINGS.copy({.isSelected = imgui_is_popup_open(IMGUI_SETTINGS.popup)}), self);

  /*
  if (imgui_begin_popup(IMGUI_SETTINGS.popup, self, IMGUI_SETTINGS.popupSize)) {
    if (_imgui_checkbox_selectable(IMGUI_VSYNC, self, self->settings->isVsync))
      window_vsync_set(self->settings->isVsync);
    _imgui_selectable(IMGUI_HOTKEYS, self);
    if (_imgui_selectable(IMGUI_DEFAULT_SETTINGS, self))
  ...
  if (imgui_begin_popup_modal(IMGUI_HOTKEYS.popup, self, IMGUI_HOTKEYS.popupSize)) {
    ...
    if (_imgui_begin_table(IMGUI_HOTKEYS_TABLE, self)) {
      static int selectedIndex = INDEX_NONE;
      ...
      for (int i = 0; i < HOTKEY_COUNT; i++) {
        ...
        if (ImGui::Selectable(chordString.c_str(), isSelected))
          selectedIndex = i;
        ImGui::PopID();
    ...
    _imgui_end_child(IMGUI_HOTKEYS_CHILD, self);

    _imgui_begin_child(IMGUI_HOTKEYS_OPTIONS_CHILD, self);
    if (_imgui_button(IMGUI_HOTKEYS_CONFIRM, self))
      imgui_close_current_popup(self);
    _imgui_end_child(IMGUI_HOTKEYS_OPTIONS_CHILD, self);

    imgui_end_popup(self);
  }

  _imgui_end(taskbar, self);
}

static void _imgui_tools(Imgui* self) {
  IMGUI_BEGIN_OR_RETURN(IMGUI_TOOLS, self);

  float displayScale = self->settings->displayScale;
  float availableWidth = ImGui::GetContentRegionAvail().x;
  float usedWidth = ImGui::GetStyle().FramePadding.x;

  for (int i = 0; i < TOOL_COUNT; i++) {

    ImguiItem item = IMGUI_TOOL_ITEMS[i]->copy(
        {.scale = displayScale,
         .isDisabled = (i == TOOL_UNDO && self->snapshots->undoStack.empty()) || (i == TOOL_REDO && self->snapshots->redoStack.empty()),
         .isSelected = self->settings->tool == (ToolType)i});
...
static void _imgui_layers(Imgui* self) {
  ...
  vec2 footerSize = {0, (ImGui::GetTextLineHeightWithSpacing() + ImGui::GetStyle().FramePadding.y * 2.0f + ImGui::GetStyle().WindowPadding.y * 2.0f)};
  vec2 childSize = ImGui::GetContentRegionAvail() - footerSize;

  ImguiItem layersChild = IMGUI_LAYERS_CHILD.copy({.size = childSize});
  _imgui_begin_child(layersChild, self);

  for (auto& [id, layer] : self->anm2->layers) {
    ImGui::PushID(id);

    ImguiItem layerItem = IMGUI_LAYER.copy({.label = std::format(IMGUI_LAYER_FORMAT, id, layer.name), .isSelected = self->selectedLayerID == id, .id = id});
    if (_imgui_atlas_selectable_input_text(layerItem, self, layer.name))
      self->selectedLayerID = id;

    /*
    ImguiItem spritesheetItem =
        IMGUI_LAYER_SPRITESHEET_ID.copy({.label = std::format(IMGUI_LAYER_FORMAT, id, layer.name), .isSelected = self->selectedLayerID == id, .id = id});
  ...
  _imgui_end_child(layersChild, self);

  ImguiItem footerChild = IMGUI_FOOTER_CHILD.copy({.size = footerSize});
  _imgui_begin_child(footerChild, self);

  if (_imgui_button(IMGUI_LAYER_ADD.copy({.isDisabled = self->anm2->path.empty()}), self))
    self->selectedLayerID = anm2_layer_add(self->anm2);

  if (_imgui_button(IMGUI_LAYER_REMOVE.copy({.isDisabled = self->selectedLayerID == ID_NONE}), self)) {
    anm2_layer_remove(self->anm2, self->selectedLayerID);
    self->selectedLayerID = ID_NONE;
  }

  _imgui_end_child(footerChild, self);
  _imgui_end(IMGUI_LAYERS, self);
}
...
static void _imgui_nulls(Imgui* self) {
  ...
  ImguiItem nullsChild = IMGUI_NULLS_CHILD.copy({.size = childSize});
  _imgui_begin_child(nullsChild, self);

  for (auto& [id, null] : self->anm2->nulls) {
    ImGui::PushID(id);

    ImguiItem nullItem = IMGUI_NULL.copy({.label = std::format(IMGUI_NULL_FORMAT, id, null.name), .isSelected = self->selectedNullID == id, .id = id});

    if (_imgui_atlas_selectable_input_text(nullItem, self, null.name))
      self->selectedNullID = id;

    ImGui::PopID();
  };

  _imgui_end_child(nullsChild, self); // nullsChild

  ImguiItem footerChild = IMGUI_FOOTER_CHILD.copy({.size = footerSize});
  _imgui_begin_child(footerChild, self);

  if (_imgui_button(IMGUI_NULL_ADD.copy({.isDisabled = self->anm2->path.empty()}), self))
    self->selectedNullID = anm2_null_add(self->anm2);

  if (_imgui_button(IMGUI_NULL_REMOVE.copy({.isDisabled = self->selectedNullID == ID_NONE}), self)) {
    anm2_null_remove(self->anm2, self->selectedNullID);
    self->selectedNullID = ID_NONE;
  }

  _imgui_end_child(footerChild, self);
  _imgui_end(IMGUI_NULLS, self);
}
...
static void _imgui_animations(Imgui* self) {
  ...
  vec2 footerSize = {0, (ImGui::GetTextLineHeightWithSpacing() + ImGui::GetStyle().FramePadding.y * 2.0f + ImGui::GetStyle().WindowPadding.y * 2.0f)};
  vec2 childSize = ImGui::GetContentRegionAvail() - footerSize;
  ImguiItem animationsChild = IMGUI_ANIMATIONS_CHILD.copy({.size = childSize});

  _imgui_begin_child(animationsChild, self);

  for (auto [i, animation] : std::views::enumerate(self->anm2->animations)) {
    ImGui::PushID(i);

    bool isDefaultAnimation = self->anm2->defaultAnimation == animation.name;
    bool isReferenceAnimation = self->reference->animationIndex == i;

    ImguiItem animationItem = IMGUI_ANIMATION.copy({.label = animation.name,
                                                    .tooltip = animation.name,
                                                    .size = vec2(childSize.x, 0),
                                                    .isSelected = self->selectedAnimationIndices.contains(i),
                                                    .id = i,
                                                    .font = isDefaultAnimation && isReferenceAnimation ? self->fonts[FONT_BOLD_ITALICS]
                                                            : isDefaultAnimation                       ? self->fonts[FONT_BOLD]
                                                            : isReferenceAnimation                     ? self->fonts[FONT_ITALICS]
                                                                                                       : self->fonts[FONT_REGULAR]});

    if (_imgui_atlas_selectable_input_text(animationItem, self, animation.name)) {
      set_list(self->selectedAnimationIndices, (int)i, ImGui::IsKeyDown(ImGuiMod_Ctrl), ImGui::IsKeyDown(ImGuiMod_Shift), &self->lastAnimationIndex);
      if (!ImGui::IsKeyDown(ImGuiMod_Ctrl) && !ImGui::IsKeyDown(ImGuiMod_Shift))
        *self->reference = {(int)i};
    }

    if (ImGui::IsItemHovered()) {
      self->clipboard->type = CLIPBOARD_ANIMATION;
      self->clipboard->destination = (int)i;
      self->clipboard->source = self->selectedAnimationIndices;
    }

    if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_None)) {
      ImGui::SetDragDropPayload(animationItem.drag_drop_get(), &i, sizeof(int));
      _imgui_atlas_selectable(animationItem.copy({.size = vec2(childSize.x, 0)}), self);
      ImGui::EndDragDropSource();
    }
    ...
    if (ImGui::BeginDragDropTarget()) {
      if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload(animationItem.drag_drop_get())) {
        ...
        if (sourceIndex != i) {
          imgui_snapshot(self, IMGUI_ACTION_ANIMATION_MOVE);
          vector_move(self->anm2->animations, sourceIndex, i);
        }
      }
  ...
  _imgui_end_child(animationsChild, self);
  ...
  if (_imgui_button(IMGUI_ANIMATION_ADD.copy({.isDisabled = self->anm2->path.empty()}), self))
    self->reference->animationIndex = anm2_animation_add(self->anm2);

  if (_imgui_button(IMGUI_ANIMATION_DUPLICATE.copy({.isDisabled = !animation}), self))
    self->reference->animationIndex = anm2_animation_add(self->anm2, animation, self->reference->animationIndex);

  _imgui_button(IMGUI_ANIMATION_MERGE.copy({.isDisabled = !animation || self->reference->animationIndex == INDEX_NONE}), self);
  ...
  if (imgui_begin_popup_modal(IMGUI_ANIMATION_MERGE.popup, self, IMGUI_ANIMATION_MERGE.popupSize)) {
    ...
    static int& mergeType = self->settings->mergeType;
    static std::set<int> selectedMergeIndices;
    static int lastMergeAnimationIndex = INDEX_NONE;
    ...
    for (auto [i, animation] : std::views::enumerate(self->anm2->animations)) {
      ...
      if (_imgui_atlas_selectable(IMGUI_ANIMATION.copy({.label = animation.name, .isSelected = selectedMergeIndices.contains(i)}), self)) {
        set_list(selectedMergeIndices, (int)i, ImGui::IsKeyDown(ImGuiMod_Ctrl), ImGui::IsKeyDown(ImGuiMod_Shift), &lastMergeAnimationIndex);
      }

      ImGui::PopID();
    }

    _imgui_end_child(IMGUI_MERGE_ANIMATIONS_CHILD, self);
    ...
    _imgui_end_child(IMGUI_MERGE_ON_CONFLICT_CHILD, self);
    ...
    _imgui_end_child(IMGUI_MERGE_OPTIONS_CHILD, self);

    if (_imgui_button(IMGUI_MERGE_CONFIRM.copy({.isDisabled = selectedMergeIndices.empty()}), self)) {
      anm2_animation_merge(self->anm2, self->reference->animationIndex, self->selectedAnimationIndices, (Anm2MergeType)mergeType);

      if (isDeleteAnimationsAfter)
        anm2_animations_remove(self->anm2, selectedMergeIndices);

      selectedMergeIndices.clear();
      imgui_close_current_popup(self);
    }
  ...
  if (_imgui_button(IMGUI_ANIMATION_REMOVE.copy({.isDisabled = !animation}), self)) {
    anm2_animations_remove(self->anm2, self->selectedAnimationIndices);
    *self->reference = Anm2Reference();
    self->selectedAnimationIndices = {};
  }

  if (_imgui_button(IMGUI_ANIMATION_DEFAULT.copy({.isDisabled = !animation}), self))
    self->anm2->defaultAnimation = animation->name;

  _imgui_end_child(footerChild, self);
  _imgui_end(IMGUI_ANIMATIONS, self);
}
...
static void _imgui_events(Imgui* self) {
  ...
  vec2 footerSize = {0, (ImGui::GetTextLineHeightWithSpacing() + ImGui::GetStyle().FramePadding.y * 2.0f + ImGui::GetStyle().WindowPadding.y * 2.0f)};
  vec2 childSize = ImGui::GetContentRegionAvail() - footerSize;
  ImguiItem eventsChild = IMGUI_EVENTS_CHILD.copy({.size = childSize});
  _imgui_begin_child(eventsChild, self);

  for (auto [id, event] : self->anm2->events) {
    ImGui::PushID(id);

    ImguiItem eventItem = IMGUI_EVENT.copy({.label = event.name, .isSelected = id == self->selectedEventID, .id = id});

    if (_imgui_atlas_selectable_input_text(eventItem, self, event.name))
      self->selectedEventID = id;

    ImGui::PopID();
  }

  _imgui_end_child(eventsChild, self);

  ImguiItem footerChild = IMGUI_FOOTER_CHILD.copy({.size = footerSize});
  _imgui_begin_child(footerChild, self);

  if (_imgui_button(IMGUI_EVENTS_ADD.copy({.isDisabled = self->anm2->path.empty()}), self)) {
    int id = map_next_id_get(self->anm2->events);
    self->anm2->events[id] = Anm2Event{};
    self->selectedEventID = id;
  }

  std::unordered_set<int> eventIDs = map_keys_to_set(self->anm2->events);

  std::unordered_set<int> usedEventIDs;
  for (auto& animation : self->anm2->animations)
    for (auto& frame : animation.triggers.frames)
      if (frame.eventID != ID_NONE)
        usedEventIDs.insert(frame.eventID);

  std::unordered_set<int> unusedEventIDs = set_symmetric_difference(eventIDs, usedEventIDs);

  if (_imgui_button(IMGUI_EVENTS_REMOVE_UNUSED.copy({.isDisabled = unusedEventIDs.empty()}), self)) {
    for (auto& id : unusedEventIDs)
      self->anm2->events.erase(id);
    imgui_log_push(self, IMGUI_LOG_REMOVE_UNUSED_EVENTS);
  }

  _imgui_end_child(footerChild, self);
  _imgui_end(IMGUI_EVENTS, self);
}
...
static void _imgui_spritesheets(Imgui* self) {
  ...
  float displayScale = self->settings->displayScale;
  vec2 footerSize = {0, ((ImGui::GetTextLineHeightWithSpacing() * 2.0f) + ImGui::GetStyle().FramePadding.y * 2.0f + ImGui::GetStyle().WindowPadding.y * 2.0f +
                         ImGui::GetStyle().ItemSpacing.y)};
  vec2 childSize = ImGui::GetContentRegionAvail() - footerSize;
  vec2 spritesheetSize = *IMGUI_SPRITESHEET_PREVIEW.size * displayScale;

  _imgui_begin_child(IMGUI_SPRITESHEETS_CHILD.copy({.size = childSize}), self);

  std::function<void(int, Anm2Spritesheet&)> spritesheet_draw = [&](int id, Anm2Spritesheet& spritesheet) {
    ImGui::PushID(id);
    vec2 itemSize = {childSize.x, spritesheetSize.y};
    _imgui_begin_child(IMGUI_SPRITESHEET_CHILD.copy({.size = itemSize}), self);

    Texture& texture = spritesheet.texture;
    bool& isInvalid = texture.isInvalid;

    ImVec2 cursorPos = ImGui::GetCursorPos();

    if (_imgui_selectable(IMGUI_SPRITESHEET.copy({.tooltip = spritesheet.path, .size = itemSize, .isSelected = self->selectedSpritesheetIDs.contains(id)}),
                          self))
      set_list(self->selectedSpritesheetIDs, id, ImGui::IsKeyDown(ImGuiMod_Ctrl), ImGui::IsKeyDown(ImGuiMod_Shift), &self->lastSpritesheetID);

    if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_None)) {
      ImGui::SetDragDropPayload(IMGUI_SPRITESHEET.drag_drop_get(), &id, sizeof(int));
      spritesheet_draw(id, spritesheet);
      ImGui::EndDragDropSource();
    }
    ...
    if (ImGui::BeginDragDropTarget()) {
      if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload(IMGUI_SPRITESHEET.drag_drop_get())) {
        ...
        if (sourceID != id) {
          imgui_snapshot(self, IMGUI_ACTION_SPRITESHEET_SWAP);
    ...
    vec2 spritesheetPreviewSize = spritesheetSize;
    float spritesheetAspect = (float)texture.size.x / texture.size.y;

    if ((spritesheetSize.x / spritesheetSize.y) > spritesheetAspect)
      spritesheetPreviewSize.x = spritesheetSize.y * spritesheetAspect;
    else
      spritesheetPreviewSize.y = spritesheetSize.x / spritesheetAspect;

    _imgui_image(IMGUI_SPRITESHEET_PREVIEW.copy({.textureID = isInvalid ? self->resources->atlas.id : spritesheet.texture.id,
                                                 .size = spritesheetPreviewSize,
                                                 .uvMin = isInvalid ? ATLAS_UV_MIN(ATLAS_NONE) : vec2(),
                                                 .uvMax = isInvalid ? ATLAS_UV_MAX(ATLAS_NONE) : vec2(1.0f),
                                                 .cursorPosition = cursorPos}),
                 self);

    _imgui_atlas_text(
        IMGUI_SPRITESHEET_TEXT.copy({.label = std::format(IMGUI_SPRITESHEET_FORMAT, id, spritesheet.path),
                                     .cursorPosition = vec2(spritesheetSize.x, (spritesheetSize.y * 0.5f) - (ImGui::GetTextLineHeight() * 0.5f))}),
        self);

    _imgui_end_child(IMGUI_SPRITESHEET_CHILD, self);
    ImGui::PopID();
  };
  ...
  _imgui_end_child(IMGUI_SPRITESHEETS_CHILD, self);

  _imgui_begin_child(IMGUI_SPRITESHEETS_FOOTER_CHILD.copy({.size = footerSize}), self);

  if (_imgui_button(IMGUI_SPRITESHEET_ADD.copy({.isDisabled = self->anm2->path.empty()}), self))
  ...
  if (_imgui_button(IMGUI_SPRITESHEETS_RELOAD.copy({.isDisabled = self->selectedSpritesheetIDs.empty()}), self)) {
    for (auto& id : self->selectedSpritesheetIDs) {
      WorkingDirectory workingDirectory(self->anm2->path);
      Texture texture;
  ...
  if (_imgui_button(IMGUI_SPRITESHEETS_REPLACE.copy({.isDisabled = self->selectedSpritesheetIDs.size() != 1}), self))
    dialog_spritesheet_replace(self->dialog, *self->selectedSpritesheetIDs.begin());
  ...
  if (self->dialog->isSelected && self->dialog->type == DIALOG_SPRITESHEET_REPLACE) {
    ...
    imgui_log_push(self, std::format(IMGUI_LOG_REPLACE_SPRITESHEET, self->dialog->replaceID, spritesheetPath));
    dialog_reset(self->dialog);
  }

  std::unordered_set<int> spritesheetIDs = map_keys_to_set(self->anm2->spritesheets);

  std::unordered_set<int> usedSpritesheetIDs;
  for (auto& [layerID, layer] : self->anm2->layers)
    usedSpritesheetIDs.insert(layer.spritesheetID);

  std::unordered_set<int> unusedSpritesheetIDs = set_symmetric_difference(spritesheetIDs, usedSpritesheetIDs);

  if (_imgui_button(IMGUI_SPRITESHEETS_REMOVE_UNUSED.copy({.isDisabled = unusedSpritesheetIDs.empty()}), self)) {
    for (auto& id : unusedSpritesheetIDs) {
      texture_free(&self->anm2->spritesheets[id].texture);
      self->anm2->spritesheets.erase(id);
    }
    imgui_log_push(self, IMGUI_LOG_REMOVE_UNUSED_SPRITESHEETS);
  }

  if (_imgui_button(IMGUI_SPRITESHEETS_SELECT_ALL.copy({.isDisabled = self->selectedSpritesheetIDs.size() == self->anm2->spritesheets.size()}), self))
    for (auto [id, _] : self->anm2->spritesheets)
      self->selectedSpritesheetIDs.insert(id);

  if (_imgui_button(IMGUI_SPRITESHEETS_SELECT_NONE.copy({.isDisabled = self->selectedSpritesheetIDs.empty()}), self))
    self->selectedSpritesheetIDs.clear();

  if (_imgui_button(IMGUI_SPRITESHEET_SAVE.copy({.isDisabled = self->selectedSpritesheetIDs.empty()}), self)) {
    for (auto& id : self->selectedSpritesheetIDs) {
      WorkingDirectory workingDirectory(self->anm2->path);
  ...
  _imgui_end_child(IMGUI_SPRITESHEETS_FOOTER_CHILD, self);
  _imgui_end(IMGUI_SPRITESHEETS, self);
}

/*
static void _imgui_animation_preview(Imgui* self) {
  static int& tool = self->settings->tool;
  static float& zoom = self->settings->previewZoom;
  ...
  if (_imgui_button(IMGUI_ANIMATION_PREVIEW_CENTER_VIEW.copy({.isDisabled = pan == vec2()}), self))
    pan = vec2();
  if (_imgui_button(IMGUI_ANIMATION_PREVIEW_FIT.copy({.isDisabled = self->reference->animationID == ID_NONE}), self)) {
    ...
    if (rect != vec4(-1.0f) && (rect.z > 0 && rect.w > 0)) {
      float scaleX = self->preview->canvas.size.x / rect.z;
      float scaleY = self->preview->canvas.size.y / rect.w;
  ...
  ImGui::Text("%s", mousePositionString.c_str());
  ...
  if (self->preview->animationOverlayID != ID_NONE)
    animationIndex = std::find(animationIndices.begin(), animationIndices.end(), self->preview->animationOverlayID) - animationIndices.begin();

  if (_imgui_combo(animationOverlayItem, self, &animationIndex))
    self->preview->animationOverlayID = animationIndices[animationIndex];
  ...
  if (self->settings->previewIsTriggers) {
    Anm2Frame trigger;
    anm2_frame_from_time(self->anm2, &trigger, {self->reference->animationID, ANM2_TRIGGER}, self->preview->time);

    if (trigger.eventID != ID_NONE) {
      ImVec2 textPos = previewCursorScreenPos + ImGui::GetStyle().ItemSpacing;
      ImGui::PushFont(NULL, ImGui::GetStyle().FontSizeBase * IMGUI_TRIGGERS_FONT_SCALE);
  ...
  float step = isMod ? TOOL_STEP_MOD : TOOL_STEP;
  ...
  if (mouseWheel != 0 || isZoomIn || isZoomOut) {
    float delta = (mouseWheel > 0 || isZoomIn) ? CANVAS_ZOOM_STEP : -CANVAS_ZOOM_STEP;
...
static void _imgui_spritesheet_editor(Imgui* self) {
  ...
  static float& zoom = self->settings->editorZoom;
  ...
  _imgui_begin_child(IMGUI_CANVAS_VIEW_CHILD, self);
  _imgui_drag_float(IMGUI_CANVAS_ZOOM, self, zoom);
  if (_imgui_button(IMGUI_SPRITESHEET_EDITOR_CENTER_VIEW.copy({.isDisabled = pan == vec2()}), self))
    pan = vec2();
  if (_imgui_button(IMGUI_SPRITESHEET_EDITOR_FIT.copy({.isDisabled = self->editor->spritesheetID == ID_NONE}), self)) {
    ...
    if ((rect.z > 0 && rect.w > 0)) {
      float scaleX = self->editor->canvas.size.x / rect.z;
      float scaleY = self->editor->canvas.size.y / rect.w;
      float fitScale = std::min(scaleX, scaleY);
  ...
  const bool isMouseClick = ImGui::IsMouseClicked(ImGuiMouseButton_Left);
  const bool isMouseRightClick = ImGui::IsMouseClicked(ImGuiMouseButton_Right);
  const bool isMouseDown = ImGui::IsMouseDown(ImGuiMouseButton_Left);
  const bool isMouseRightDown = ImGui::IsMouseDown(ImGuiMouseButton_Right);
  const bool isMouseMiddleDown = ImGui::IsMouseDown(ImGuiMouseButton_Middle);
  const bool isZoomIn = _imgui_chord_pressed(imgui_hotkey_chord_registry()[HOTKEY_ZOOM_IN]);
  const bool isZoomOut = _imgui_chord_pressed(imgui_hotkey_chord_registry()[HOTKEY_ZOOM_OUT]);
  const float mouseWheel = ImGui::GetIO().MouseWheel;
  ...
  float step = isShift ? TOOL_STEP_MOD : TOOL_STEP;

  if ((tool == TOOL_PAN && isMouseDown) || isMouseMiddleDown)
    pan += vec2(mouseDelta.x, mouseDelta.y);

  auto spritesheet_editor_move = [&](bool isPrimary = true) {
    bool isActivated = isPrimary ? isMouseClick || isLeft || isRight || isUp || isDown : isMouseRightClick;
    bool isUseMouseDown = isPrimary ? isMouseDown : isMouseRightDown;
  ...
  auto spritesheet_editor_crop = [&](bool isPrimary = true) {
    bool isActivated = isPrimary ? isMouseClick || isLeft || isRight || isUp || isDown : isMouseRightClick;
    bool isUseMouseClick = isPrimary ? isMouseClick : isMouseRightClick;
    bool isUseMouseDown = isPrimary ? isMouseDown : isMouseRightDown;

    vec2 cropPosition = self->settings->editorIsGridSnap ? vec2(roundf(position.x / gridSize.x) * gridSize.x + gridOffset.x - (gridSize.x * 0.5f),
                                                                roundf(position.y / gridSize.y) * gridSize.y + gridOffset.y - (gridSize.y * 0.5f))
                                                         : position;
  ...
  auto spritesheet_editor_draw = [&](vec4 color) {
    if (isMouseClick || isMouseRightClick)
      imgui_snapshot(self, color != vec4() ? IMGUI_ACTION_DRAW : IMGUI_ACTION_ERASE);
  ...
  case TOOL_MOVE:
    if (!texture || !frame)
      break;
    spritesheet_editor_move(true);
    spritesheet_editor_crop(false);
    break;
  case TOOL_CROP:
    if (!texture || !frame)
      break;
    spritesheet_editor_crop(true);
    spritesheet_editor_move(false);
    break;
  case TOOL_DRAW:
  case TOOL_ERASE:
    if (!texture)
      break;
    spritesheet_editor_draw(tool == TOOL_DRAW && !isMouseRightDown ? self->settings->toolColor : vec4());
    break;
  case TOOL_COLOR_PICKER:
    if (isMouseDown) {
      ...
      ImGuiIO& io = ImGui::GetIO();
      ivec2 fbPosition = {(int)(position.x * io.DisplayFramebufferScale.x), (int)(position.y * io.DisplayFramebufferScale.y)};
      ivec2 size{};
      SDL_GetWindowSizeInPixels(self, &size.x, &size.y);

      if (fbPosition.x < 0 || fbPosition.y < 0 || fbPosition.x >= size.x || fbPosition.y >= size.y)
        return false;
      ...
      color = vec4(UINT8_TO_FLOAT(rgba[0]), UINT8_TO_FLOAT(rgba[1]), UINT8_TO_FLOAT(rgba[2]), UINT8_TO_FLOAT(rgba[3]));
      ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2());
      ImGui::BeginTooltip();
...
static void _imgui_frame_properties(Imgui* self) {
  ...
  if (!frame || type != ANM2_TRIGGER) {
    _imgui_drag_float2(IMGUI_FRAME_PROPERTIES_CROP.copy({.isDisabled = !isLayerFrame}), self, !isLayerFrame ? dummy_value<vec2>() : frame->crop);
    _imgui_drag_float2(IMGUI_FRAME_PROPERTIES_SIZE.copy({.isDisabled = !isLayerFrame}), self, !isLayerFrame ? dummy_value<vec2>() : frame->size);
    _imgui_drag_float2(IMGUI_FRAME_PROPERTIES_POSITION.copy({.isDisabled = !frame}), self, !frame ? dummy_value<vec2>() : frame->position);
    _imgui_drag_float2(IMGUI_FRAME_PROPERTIES_PIVOT.copy({.isDisabled = !isLayerFrame}), self, !isLayerFrame ? dummy_value<vec2>() : frame->pivot);
    _imgui_drag_float2(IMGUI_FRAME_PROPERTIES_SCALE.copy({.isDisabled = !frame}), self, !frame ? dummy_value<vec2>() : frame->scale);
    _imgui_drag_float(IMGUI_FRAME_PROPERTIES_ROTATION.copy({.isDisabled = !frame}), self, !frame ? dummy_value<float>() : frame->rotation);
    _imgui_input_int(IMGUI_FRAME_PROPERTIES_DELAY.copy({.isDisabled = !frame}), self, !frame ? dummy_value<int>() : frame->delay);
    _imgui_color_edit4(IMGUI_FRAME_PROPERTIES_TINT.copy({.isDisabled = !frame}), self, !frame ? dummy_value<vec4>() : frame->tintRGBA);
    _imgui_color_edit3(IMGUI_FRAME_PROPERTIES_COLOR_OFFSET.copy({.isDisabled = !frame}), self, !frame ? dummy_value<vec3>() : frame->offsetRGB);
    _imgui_checkbox(IMGUI_FRAME_PROPERTIES_VISIBLE.copy({.isDisabled = !frame}), self, !frame ? dummy_value<bool>() : frame->isVisible);
    _imgui_checkbox(IMGUI_FRAME_PROPERTIES_INTERPOLATED.copy({.isDisabled = !frame}), self, !frame ? dummy_value<bool>() : frame->isInterpolated);
    _imgui_checkbox(IMGUI_FRAME_PROPERTIES_ROUND.copy({.isDisabled = !frame}), self, self->settings->propertiesIsRound);
    if (_imgui_button(IMGUI_FRAME_PROPERTIES_FLIP_X.copy({.isDisabled = !frame}), self))
      frame->scale.x = -frame->scale.x;
    if (_imgui_button(IMGUI_FRAME_PROPERTIES_FLIP_Y.copy({.isDisabled = !frame}), self))
    ...
    ImguiItem eventItem = IMGUI_FRAME_PROPERTIES_EVENT.copy({.isDisabled = !frame});

    eventItem.items[ID_NONE] = IMGUI_NONE;

    for (auto& [id, event] : self->anm2->events)
      eventItem.items[id] = event.name;

    _imgui_combo(eventItem, self, frame->eventID);
    _imgui_input_int(IMGUI_FRAME_PROPERTIES_AT_FRAME.copy({.isDisabled = !frame}), self, frame->atFrame);
  }

  _imgui_end(IMGUI_FRAME_PROPERTIES, self);
}
...
static void _imgui_log(Imgui* self) {
  ...
  float itemSpacingHeight = ImGui::GetStyle().ItemSpacing.y;

  ImVec2 position = {io.DisplaySize.x - itemSpacingHeight, io.DisplaySize.y - itemSpacingHeight};

  for (int i = (int)self->log.size() - 1; i >= 0; --i) {
    ImguiLogItem& item = self->log[i];
    float lifetime = item.timeRemaining / IMGUI_LOG_DURATION;
    ...
    ImguiItem logItem = IMGUI_LOG_WINDOW.copy({.label = std::format(IMGUI_LOG_FORMAT, i)});
    _imgui_begin(logItem, self);
    ImGui::TextUnformatted(item.text.c_str());
    ImVec2 windowSize = ImGui::GetWindowSize();
    _imgui_end(logItem, self); // IMGUI_LOG_WINDOW

    ImGui::PopStyleColor(2);

    position.y -= windowSize.y + ImGui::GetStyle().ItemSpacing.y;
  }
}
...
static void _imgui_dock(Imgui* self) {
  ...
  float windowOffset = IMGUI_TASKBAR.size->y * self->settings->displayScale;
  vec2 windowPosition = {viewport->Pos.x, viewport->Pos.y + windowOffset};
  vec2 windowSize = {viewport->Size.x, viewport->Size.y - windowOffset};

  ImGui::SetNextWindowPos(windowPosition);
  ImGui::SetNextWindowSize(windowSize);
  ImGui::SetNextWindowViewport(viewport->ID);

  _imgui_begin(IMGUI_WINDOW_MAIN, self);
  ...
  _imgui_end(IMGUI_WINDOW_MAIN, self);
}
...
void imgui_init(Imgui* self, Dialog* dialog, Resources* resources, Anm2* anm2, Anm2Reference* reference, Editor* editor, Preview* preview,
  ...
  ImGui::GetStyle().FontScaleMain = self->settings->displayScale;
  ImFontConfig config;
  config.FontDataOwnedByAtlas = false;

  for (auto [i, font] : std::views::enumerate(FONTS))
    self->fonts[i] = io.Fonts->AddFontFromMemoryTTF((void*)font.data, font.length, FONT_SIZE, &config);
  ...
  if (self->isContextualActionsEnabled) {
    for (const auto& item : imgui_item_registry()) {
      if (!item->is_chord() || !_imgui_chord_pressed(item->chord_get()))
        continue;
      if (item->isAllowHotkeyWhenFocusWindow && !_imgui_is_focus_window(*item->focusWindow))
        continue;
      if (!item->isAllowHotkeyWhenFocusWindow && item->focusWindow.has_value())
        continue;
      if (item->snapshotAction)
        imgui_snapshot(self, *item->snapshotAction);
      if (item->function)
        item->function(self);
    }
  }
