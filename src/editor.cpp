...
void editor_draw(Editor* self)
{
    ...
    if (self->spritesheetID != ID_NONE)
    {
        ...
        mat4 spritesheetTransform = transform * quad_model_get(texture.size, {}, {}, {}, vec2(1.0f));
        ...
        if (frame)
        {
            mat4 cropTransform = transform * quad_model_get(frame->size, frame->crop, {}, {}, vec2(1.0f));
            canvas_rect_draw(&self->canvas, shaderLine, cropTransform, EDITOR_FRAME_COLOR);

            mat4 pivotTransform = transform * quad_model_get(CANVAS_PIVOT_SIZE, frame->crop + frame->pivot, CANVAS_PIVOT_SIZE * 0.5f, {}, vec2(1.0f));
