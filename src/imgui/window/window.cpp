...
namespace anm2ed::imgui
{
                              ...
                              const std::set<int>* quickGroupSelection = nullptr)
  {
    ...
    if (options.selection.empty())
    {
      ...
      if ((overlayDocumentId == 0 || overlayDocumentId == document.tabId) && selected.contains(overlayIndex))
      {
        overlayIndex = -1;
        overlayDocumentId = 0;
      }
    ...
    else
    {
      ...
      if ((overlayDocumentId == 0 || overlayDocumentId == document.tabId) && mergeSelection.contains(overlayIndex))
      {
        overlayIndex = -1;
        overlayDocumentId = 0;
      }
    ...
    window.remove = [](Window& window, Manager&, Settings&, Document& document, Clipboard&)
    {
      ...
      auto behavior = [&]()
      {
        ...
        for (auto it = selection.rbegin(); it != selection.rend(); ++it)
        {
          ...
          if ((overlayDocumentId == 0 || overlayDocumentId == document.tabId) && overlayIndex == i)
          {
            overlayIndex = -1;
            overlayDocumentId = 0;
          }
    ...
    window.cut = [](Window& window, Manager& manager, Settings& settings, Document& document, Clipboard& clipboard)
    {
      ...
      auto behavior = [&]()
      {
        ...
        for (auto it = selectedIndices.rbegin(); it != selectedIndices.rend(); ++it)
        {
          ...
          auto childIndex = window_animation_child_index_get(*items, i);
          if (childIndex == -1) continue;
          if ((document.overlayDocumentId == 0 || document.overlayDocumentId == document.tabId) &&
              document.overlayIndex == i)
          {
